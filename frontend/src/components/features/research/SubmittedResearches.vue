      ...
      <div class="overflow-x-auto flex-1">
        <table class="min-w-full divide-y divide-gray-100 table-fixed">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/3">Title</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/6">
                {{ isArchived ? 'Auto-Delete' : 'Timeline' }}
              </th>
              <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/6">Details</th>
              <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/6">Status</th>
              <th class="px-6 py-3 text-right text-xs font-bold text-gray-500 uppercase tracking-wider w-1/6">Actions</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-50">
            <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-emerald-50/60 transition cursor-pointer group" @click="$emit('view', item)">
              ...
              <td class="px-6 py-4">
                <div v-if="isArchived">
                  <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded bg-red-50 text-red-700 border border-red-100 whitespace-nowrap">
                    <span>⚠️</span> {{ getArchiveDaysLeft(item.archived_at) }} Days left
                  </span>
                </div>
